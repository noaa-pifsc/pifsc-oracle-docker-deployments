...
function prepare_execute_deployment_script ()
{
	...
	# input validation
    if [[ -z "$env_name" || -z "$deployment_destination" || -z "$container_hostname" || -z "$container_host_project_path" || -z "$container_git_url" || -z "$config_var_name" || -z "$container_host_scripts_path" || -z "$local_container_build_path" || -z "$container_compose_file_path" || -z "$script_type" || -z "$db_host" || -z "$db_service" || -z "$secret_mapping_var" ]]; then
